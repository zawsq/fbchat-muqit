...
class MessengerClient:
    ...
    async def fetch_thread_info(self, thread_ids: List[str]) -> Tuple[Thread, ...]:
        ...
        if self._state:
            try:
                result = await self._state._post(
    ...
    async def fetch_thread_list(
        self,
        limit: int = 5,
        thread_folder: ThreadFolder = ThreadFolder.INBOX,
        ...
        if self._state:
            try:
                ...
                return parse_thread_info(
                    result[0]["viewer"]["message_threads"]["nodes"]
                )
            except Exception as e:
    ...
    async def fetch_thread_messages(
        self, thread_id: str, message_limit: int = 5, before: Optional[int] = None
    ) -> List[Message] | None:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    "Failed to parse fetched response for thread messages, failure could be due to changes in response format"
                )
                self.logger.debug(
                    "Failed to parse fetched response for thread messages. The response: {result}"
                )
        except Exception as e:
            self.logger.error(
                "Failed to fetch Thread Messages, Skipping it.", exc_info=e
            )
    ...
    async def fetch_message_info(
        ...
        try:
            ...
            return self._parser.parse_message_from_graphql(
                result[0]["message"], thread_id
            )
        except ParsingError as e:
            self.logger.error(str(e))
            raise
    ...
    async def fetch_thread_themes(self) -> List[Theme]:
        ...
        data = {
            ...
            "variables": json.dumps({"version": "default"}),
        ...
        r = await self._state._post(
            "https://www.facebook.com/api/graphql/", data=data, raw=True
        )
    ...
    async def send_message(
        ...
        if file_path:
            payload["send_type"] = 3
            file_ids = await self.uploadFiles(
                file_path=file_path
            )  # returns list of uploaded id
        ...
        if "Couldn't send" in response.payload:
            self.logger.error(
                "Couldn't send the message. Could it be that you have provided a `User` thread Id?"
            )
            return
        return extract_message_id_raw(response.payload)

    async def send_quick_reaction(
        self, thread_id: str, emoji: str, emojj_size: int = 3
    ):
    ...
    async def search_message(
        self, text: str, thread_id: str, thread_type: ThreadType = ThreadType.GROUP
    ) -> Dict:
    ...
    async def create_group_thread(
        self, participant_ids: List[str], emoji_sticker: str = "369239263222822"
    ) -> Optional[str]:
        ...
        for label, p in payloads:
            ...
            if self._mqtt and label == "153":
                await self._mqtt._mqttClient.publish(
                    "/ls_req", json.dumps(payload), qos=1
                )
                continue
    ...
    async def mute_thread(
        self, thread_id: str, mute_forever: bool = False, duration_ms: int = -1
    ):
        """
                                ...
                                "payload": json.dumps(
                                    {
                                        "thread_key": int(
                                            user_id
                                        )  # Using restrictee ID same as captured
                                    }
                                ),
