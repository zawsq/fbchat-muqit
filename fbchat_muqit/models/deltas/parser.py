...
class MessageParser:
    ...
    def parse_story_extensible(self, data: extensibleattachment) -> SharedAttachment:
        return SharedAttachment(
            ...
            description=str(data.story_attachment.description)
    ...
    def parse_profile_extensible(self, data) -> ProfileAttachment:
        return ProfileAttachment(
            ...
            cover_photo=data.story_attachment.target.cover_photo["photo"]["image"][
                "uri"
            ]
    ...
    def parse_all(self, topic, payload) -> Optional[ParsedEvent]:
        ...
        if (
            ...
            self.logger.debug(
                f"Unknown payload delta from Topic: '{topic}' received: {self.decoder.decode(payload)}"
            )
            return None
    ...
    def parse_message(self, data, replied_to_message=None) -> Message:
        ...
        if attachments:
            message_type = (
                self._attach_to_message[attachments[0].type]
                if attachments[0]
        ...
        return Message(
            ...
            can_unsend=True
            if data.messageMetadata.unsendType == "Can_Unsend"
            else False,
            unsent=False,
