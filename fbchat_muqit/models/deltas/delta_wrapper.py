...
class ClientPayload(
    Struct, frozen=True, eq=False, tag="ClientPayload", tag_field="class"
):
    payload: DecodedPayloadType  # receives `ClientPayloadDelta` object


class NewMessageDelta(
    Struct, frozen=True, eq=False, tag="NewMessage", tag_field="class"
):
...
DeltaTypes = Union[
    NewMessageDelta, ClientPayload, NoOpDelta, TimestampDeltas, MessengerGroupDeltas
]
