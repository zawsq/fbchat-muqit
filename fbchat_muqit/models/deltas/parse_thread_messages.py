...
thread_message_decoder = json.Decoder(
    type=List[VMessageThread], strict=False, dec_hook=extract_thread_id
)
...
def parse_thread_message(payload: bytes) -> List[Message]:
    ...
    messages = [
        Message(
            ...
            can_unsend=True
            if m["message_unsendability_status"] == "can_unsend"
            else False,
            unsent=False,
            reaction=[
                MessageReaction(
            ...
            attachments=[
                MessageParser().parse_attachment(
                    Mercury(
