...
def get_and_parse(t) -> Thread:
    ...
    return Thread(
        ...
        all_participants=tuple(
            convert(nodes["node"]["messaging_actor"], type=User, dec_hook=extractVal)
            for nodes in t["all_participants"]["edges"]
        )
        if t["thread_type"] != "ONE_TO_ONE"
        else tuple(),  # type: ignore
    )
...
def merge_nickname(data: Dict) -> Dict[str, str] | None:
    return (
        {
            dic["participant_id"]: dic["nickname"]
            for dic in data["participant_customizations"]
        }
        if data
        else None
    )
