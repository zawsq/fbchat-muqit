...
def post_attachments(
    picture_ids: Optional[List[str]] = None, video_ids: Optional[List[str]] = None
):
...
class FacebookClient:
    ...
    async def manage_friend_request(self, user_id: str, accept_request: bool = True):
        ...
        if not self._state:
            raise LoginError(
                "Client is not logged in yet. `State` class is not initialised yet"
            )
        ts = str(now())
        variables = {
            "input": {
        ...
        await self._state._post(
            "https://www.facebook.com/api/graphql/", data=data, no_response=True
        )
    ...
    async def react_to_post(
        ...
        reaction: FBReaction = FBReaction.LOVE,
    ):
        ...
        if not feedback_id and not post_id:
            raise ValidationError(
                "Either 'feedback_id' or 'post_id' must be provided to react to Facebook post."
            )

        if post_id:
            feedback_id = base64.b64encode(
                post_id.to_bytes((post_id.bit_length() + 7) // 8, byteorder="big")
            ).decode("utf-8")
    ...
    async def _get_privacy_writer(self):
        ...
        try:
            ...
            raise ParsingError(
                "Failed to parse graphql response while getting privacy writer Id to set audience type.",
                original_exception=e,
            )
        return r

    async def _setPrivacyWriter(
        self,
        ids: List,
        privacyWriter: PrivacyRow,
        isAllow=False,
        isDeny=False,
        base_state=None,
    ) -> Privacy | None:
            ...
            "variables": json.dumps(
                {
                    "localPrivacyRow": {
                        ...
                        if base_state
                        else privacyWriter.privacy_row_input.base_state.value,
    ...
    async def _format_audience(self, specific_users, except_users, base_state):
        if specific_users and except_users:
            raise APIError(
                "Provide 'specific_users' or 'except_users' only one at a time."
            )
        ...
        if (
            base_state != privacy_writer.privacy_row_input.base_state.value
            and not specific_users
            and not except_users
        ):
            await self._setPrivacyWriter([], privacy_writer, base_state=base_state)

        if specific_users:
            data = await self._setPrivacyWriter(
                specific_users, privacy_writer, isAllow=True
            )
            if data:
            ...
            data = await self._setPrivacyWriter(
                except_users, privacy_writer, isDeny=True
            )
            if data:
                specific_users = data.specific_users
                except_users = data.except_users
    ...
    async def publish_post(
        ...
        if video_paths:
            self.logger.warning(
                f"Due to some issues attaching video to post is not supported at this moment. Skipping videos."
            )
        ...
        if uploaded_photo_ids or uploaded_video_ids:
            attachments = post_attachments(
                picture_ids=uploaded_photo_ids, video_ids=uploaded_video_ids
            )

        self.logger.info(f"attachments: {attachments}")
        self.logger.info("Now getting audience...")
        audience_data = await self._format_audience(
            specific_users, except_users, audience.value
        )
        ...
        r = await self._state._post(
            "/api/graphql/",
            data=data,
            raw=True,
            header_type="publish_post",
            headers=headers,
        )
        try:
            ...
            raise ParsingError(
                "Failed to parse graphql response of post publishing. Coudn't get `feedback_id` or `post_id`",
                original_exception=e,
            )
    ...
    async def upload_photo(self, image_path: str) -> str:
        ...
        response = await self._state._post(
            url, data=data, files=files, raw=True, headers=headers
        )
    ...
    async def upload_photos(
        ...
        tasks = [
            upload_with_semaphore(path, idx) for idx, path in enumerate(image_paths)
        ]
        ...
        self.logger.info(
            f"Upload complete: {len(photo_ids)}/{len(image_paths)} successful"
        )
