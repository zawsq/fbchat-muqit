...
class GraphQLProcessor:
    ...
    def parse_json_stream(self, content: str) -> List[Dict[str, Any]]:
        ...
        while idx < len(content):
            ...
            try:
                ...
                idx += offset
                self.logger.trace(
                    f"Successfully parsed JSON object at position {idx - offset}"
                )
    ...
    def strip_json_cruft(self, content: str) -> str:
        ...
        self.logger.trace(
            f"Stripping JSON cruft from {len(content)} character response"
        )
        ...
        try:
            ...
            if cruft_length > 0:
                ...
                self.logger.debug(
                    f"Removed {cruft_length} characters of cruft: {cruft!r}"
                )
    ...
    def queries_to_json(self, *queries: QueryRequest) -> str:
        ...
        for i, query in enumerate(queries):
            if query.query:
                ...
                self.logger.trace(
                    f"Query {i}: GraphQL query with {len(query.query)} characters"
                )
            elif query.query_id:
                rtn[f"q{i}"] = {
                ...
                rtn[f"q{i}"] = {"doc": query.doc, "query_params": query.query_params}
                self.logger.trace(
                    f"Query {i}: Document with {len(query.doc)} characters"
                )
            elif query.doc_id:
                rtn[f"q{i}"] = {
                    "doc_id": query.doc_id,
        ...
        self.logger.debug(
            f"Generated JSON query string with {len(json_str)} characters"
        )
        return json_str
    ...
    def handle_payload_error(self, payload: Dict[str, Any]) -> None:
        ...
        error_details = {"error_code": error_code, "payload": payload}
    ...
    def handle_graphql_errors(self, response: Dict[str, Any]) -> None:
        ...
        self.logger.error(
            f"GraphQL error - Code: {error.code}, Message: {error.message}"
        )
    ...
    def process_response(self, content: str) -> List[Optional[Dict[str, Any]]]:
                ...
                for key, value in obj.items():
                    ...
                    try:
                        ...
                        if "response" in value:
                            ...
                            self.logger.trace(
                                f"Extracted 'response' data for query {index}"
                            )
                        elif "data" in value:
                            results[index] = value["data"]
                            self.logger.trace(
                                f"Extracted 'data' data for query {index}"
                            )
                        else:
                            results[index] = value
        ...
        self.logger.info(
            f"Successfully processed {processed_count}/{len(results)} responses"
        )
...
def from_query_id(
    query_id: str, params: Optional[Dict[str, Any]] = None
) -> QueryRequest:
